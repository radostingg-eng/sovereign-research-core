...
REQUIRED_INVARIANTS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("web_capture_origin_boundary",
     ("web prose", "`host_summary`", "`connector_response`",
      "`result_origin`, never `capture_origin`",
      "`direct_connector_response` needs exact")),
    ...
    ("worker_research_adoption",
     ("feedback.json.research_inbox", "worker_attested",
      ...
      "used_as_lead", "rejected", "deferred",
      "research_inbox.projection_id", "worker_research_projection_id",
    ...
    ("same_slot_semantic_retry",
     ("retry_contract.patch_base.path",
      "repair that copy, not the archive",
    ...
    ("duplicate_key_repair_only_source",
     ("duplicate_json_key",
      ...
      "repair-only",
      "copy the immutable",
      "to a new `host_staging/` filename",
      "never edit the archive",
      "never accepted",
    ...
    ("semantic_precommit_gate",
     ("before every staging commit", "learning_stage_dispositions",
      "never emit", "cognitive_stages")),
    ("exact_retry_lineage",
    ...
    ("expected_slot_utc",
     ("schedule_context.expected_slot", "anchor_at", "actual utc",
      "never local time")),
    ("honest_unavailable_run_id",
     ("schedule_context.schema_version", "platform_run_id: null",
      "platform_run_id_status: unavailable", "never substitute",
      "gate a", "gate b")),
