...
class RewordingIsAllowedTests(unittest.TestCase):
    ...
    def test_surrounding_prose_can_change_freely(self):
        reworded = (
            ...
            "blocker.\n"
            "When retry_contract.targets includes a duplicate_json_key entry "
            "and retry_contract.refused_source is present, treat "
            "refused_source.path as repair-only lexical scaffolding: edit "
            "that exact archived file in place to remove or merge the named "
            "duplicate top-level key so it appears exactly once, and commit "
            "the fixed document. refused_source is never accepted and does "
            "not authorise promotion; patch_base.path` remains the trusted "
            "structural comparison. Never append another occurrence of the "
            "same key.\n"
