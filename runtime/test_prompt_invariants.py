...
class RewordingIsAllowedTests(unittest.TestCase):
    ...
    def test_surrounding_prose_can_change_freely(self):
        reworded = (
            ...
            "connector returned.\n"
            "For web prose use `host_summary`. `connector_response` is "
            "`result_origin`, never `capture_origin`. "
            "`direct_connector_response` needs exact raw output.\n"
            ...
            "Reserve top-level evidence_calls for state projections; never "
            "duplicate either `market_scout_report.tool_calls` or "
            ...
            "a pass adds nothing.\n"
            "Read next_question_metrics; each next_question_id gets a selected "
            "or rejected target_missing_information_id. Age is context, never "
            "rank, and repeated deferral needs a reason.\n"
            ...
            "decision.forecast_assessment names material_premise, required or "
            "not_required, and forecast_ids; runtime never applies a "
            ...
            "used_as_lead keeps adopted_lead before expiry; "
            "rejected/deferred keep host disposition and rationale, "
            "not worker content.\n"
            ...
            "required_stage_output_fields entry, "
            ...
            "When retry_contract.refused_source exists, copy to a new "
            "staging file and repair that copy, not the archive. "
            "Use retry_contract.patch_base.path for structural comparison "
            "only; commit another corrected candidate and do not stop "
            "after the first refusal unless there is a non-recoverable "
            "blocker.\n"
            "When retry_contract.targets includes duplicate_json_key and "
            "retry_contract.refused_source is present, verify its sha256; "
            "copy the immutable refused_source.path to a NEW `host_staging/` "
            "filename and repair that copy. Never edit the archive. The "
            "refused_source is repair-only and never accepted; "
            "patch_base.path` remains the trusted structural comparison. "
            "Never append another occurrence of the same key.\n"
            "If retry_contract` is absent, use "
            "last_accepted_semantic_source or the committed schema exemplar.\n"
            "Never reuse a refused filename: a name alone cannot identify "
            "bytes. Match filename@sha256:<digest> before retrying.\n"
            "Strict-parse your own output, reject duplicate keys, and emit "
            "pretty-printed JSON with one trailing newline. Before every "
            "staging commit, retain learning_stage_dispositions and never "
            "emit generated cognitive_stages. Copy "
